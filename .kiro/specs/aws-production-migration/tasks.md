...
- [x] 1. Install AWS SDK dependencies and configure testing framework
...
- [x] 2. Create shared TypeScript types
  - [x] 2.1 Create `lib/types.ts` with all shared interfaces
...
- [x] 3. Create Auth Service
  - [x] 3.1 Create `lib/services/auth.ts` with APP_ENV switching pattern
...
- [x] 4. Create DB Service
  - [x] 4.1 Create `lib/services/db.ts` with APP_ENV switching pattern
    - At module load, check `APP_ENV` (import from `lib/types.ts` or inline `const isLocal = (process.env.APP_ENV ?? 'local') !== 'production'`)
...
- [x] 5. Create AI Service
  - [x] 5.1 Create `lib/services/ai.ts` with APP_ENV switching pattern
    - At module load, check `APP_ENV` (import from `lib/types.ts` or inline `const isLocal = (process.env.APP_ENV ?? 'local') !== 'production'`)
...
- [x] 6. Create session helper
  - [x] 6.1 Create `lib/session.ts`
...
- [x] 7. Create auth API routes
  - [x] 7.1 Create `app/api/auth/login/route.ts`
  ...
  - [x] 7.2 Create `app/api/auth/signup/route.ts`
...
- [x] 8. Refactor existing API routes
  - [x] 8.1 Refactor `app/api/analyze/route.ts`
  ...
  - [x] 8.2 Refactor `app/api/chat/route.ts`
  ...
  - [x] 8.3 Refactor `app/api/game/route.ts`
  ...
  - [x] 8.6 Create `app/api/checkin/route.ts`
  ...
  - [x] 8.7 Create `app/api/community/route.ts`
...
- [x] 10. Update frontend components to use API routes
  - [x] 10.1 Update check-in flow to POST to `/api/checkin`
  ...
  - [x] 10.2 Update stats page to fetch from `/api/checkin`
    - In `app/stats/page.tsx`: replace localStorage read with `fetch('/api/checkin')` in a `useEffect`
    - _Requirements: 7.2_

  - [x] 10.3 Update game result submission to POST to `/api/game-result`
  ...
  - [x] 10.4 Update games dashboard to fetch from `/api/game`
    - In `app/games/page.tsx` or `components/games/GamesDashboard.tsx`: replace localStorage read with `fetch('/api/game')`
    - _Requirements: 7.4_

  - [x] 10.5 Update community interactions to use `/api/community`
...
- [x] 11. Create environment configuration file
...
- [x] 12. Create CDK infrastructure stack
  - [x] 12.0 Store GitHub personal access token in AWS Secrets Manager (prerequisite)
  ...
  - [x] 12.1 Scaffold `/infrastructure` directory
  ...
  - [x] 12.2 Implement `infrastructure/lib/wellness-stack.ts`
  ...
  - [x] 12.3 Create `infrastructure/bin/app.ts` CDK entry point
    - Instantiate `WellnessStack` in a CDK `App`
    - _Requirements: 6.6_

- [x] 13. Cleanup mock files and console.logs
